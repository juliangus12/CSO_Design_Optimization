...
filename<-paste(main_path,"/dataset/AMS_ellicott_temp.txt",sep='')
big<-scan(filename,skip=1,list(date=0,storms=0))
year<-big$date
LT_hist<-big$storms
time_hist<-year
# re-normalize
ind_norm <- which(time_hist==1951):which(time_hist==2018)
tempset <- (LT_hist - mean(LT_hist[ind_norm]))/sd(LT_hist[ind_norm])
...
cred.table
save(cred.table,file=paste(main_path,"/projections/results/LTRESULT/stat_widenorm_CI_LT.RData",sep=""))
save(run2,file=paste(main_path,"/projections/results/LTRESULT/stat_widenorm_run2_LT.RData",sep=""))
mu=run2$finmat[,1]
...
save(mu_chain,xi_chain,sigma_chain,file=paste(main_path,"/projections/results/LTRESULT/stat_widenorm_param_LT.RData",sep=""))
save(retint,file=paste(main_path,"/projections/results/LTRESULT/stat_widenorm_rtnlevel.RData",sep=""))
...
cred.table<-CredIntervalsGEV(run2,burn=burnin) # Credible Intervals
cred.table
save(run2,file=paste(main_path,"/projections/results/LTRESULT/Mu_nonstat_widenorm_run2_LT.RData",sep=""))
save(cred.table,file=paste(main_path,"/projections/results/LTRESULT/Mu_nonstat_widenorm_CI_LT.RData",sep=""))
mu0=run2$finmat[,1]
amu=run2$finmat[,4]
...
xi_chain <- xi[(length(xi)-40000+1):length(xi)]
retint<-benreturn(100,mu_chain,sigma_chain,xi_chain)
save(mu_chain,xi_chain,sigma_chain,file=paste(main_path,"/projections/results/LTRESULT/Mu_nonstat_widenorm_param_LT.RData",sep=""))
save(retint,file=paste(main_path,"/projections/results/LTRESULT/Mu_nonstat_widenorm_rtnlevel.RData",sep=""))
save(mu0_chain,amu0_chain,slope,file=paste(main_path,"/projections/results/LTRESULT/slope_nonstat_widenorm_param_LT.RData",sep=""))
