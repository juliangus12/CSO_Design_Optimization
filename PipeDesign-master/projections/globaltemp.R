...
infile<-paste(main_path,"/dataset/Globaltemp.txt",sep='')
big<-scan(infile,skip=76,list(date=0,temp=0))
year<-big$date
time_hist<-year
GT_hist<-big$temp
ind_norm <- which(time_hist==1951):which(time_hist==2013) 
tempset <- (GT_hist - mean(GT_hist[ind_norm]))/sd(GT_hist[ind_norm])
# (1) Stationary
...
save(cred.table,file=paste(main_path,"/projections/results/GTRESULT/stat_widenorm_CI_GT.RData",sep=""))
save(run2,file=paste(main_path,"/projections/results/GTRESULT/stat_widenorm_run2_GT.RData",sep=""))
mu=run2$finmat[,1]
...
save(mu_chain,xi_chain,sigma_chain,file=paste(main_path,"/projections/results/GTRESULT/stat_widenorm_param_GT.RData",sep=""))
save(retint,file=paste(main_path,"/projections/results/GTRESULT/stat_widenorm_rtnlevel.RData",sep=""))
# (2) Mu Non-Stationary
...
MULTncrej(run2,burn=burnin) #RejectionRate
cred.table<-CredIntervalsGEV(run2,burn=burnin) # Credible Intervals
save(run2,file=paste(main_path,"/projections/results/GTRESULT/Mu_nonstat_widenorm_run2_GT.RData",sep=""))
save(cred.table,file=paste(main_path,"/projections/results/GTRESULT/Mu_nonstat_widenorm_CI_GT.RData",sep=""))
mu0=run2$finmat[,1]
amu=run2$finmat[,4]
mu=mu0*(1+amu*tempset[63])
sigma=exp(run2$finmat[,2])
xi=run2$finmat[,3]
mu_chain <-mu[(length(xi)-40000+1):length(xi)]
...
xi_chain <- xi[(length(xi)-40000+1):length(xi)]
retint<-benreturn(100,mu_chain,sigma_chain,xi_chain)
save(mu_chain,xi_chain,sigma_chain,file=paste(main_path,"/projections/results/GTRESULT/Mu_nonstat_widenorm_param_GT.RData",sep=""))
save(retint,file=paste(main_path,"/projections/results/GTRESULT/Mu_nonstat_widenorm_rtnlevel.RData",sep=""))
